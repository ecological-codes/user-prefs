...
| `.claude/claude-sp-guards.md` | System Prompt (SP) compensation companion for Claude environments, provides memory conflict surfacing, canonization, hygiene, credential protocol. Load every session, required as long as [Sonnet and Opus SP deficiencies](#reference) persist. Adapt to other models as required. |
...
| `CLAUDE.md` | Root manifest for Claude Code. Thin file - `@import` the four source files above so Claude Code loads them as project memory. Edit the sources, not this. |
...
---
README.md v1.8.9 - Human Approved
